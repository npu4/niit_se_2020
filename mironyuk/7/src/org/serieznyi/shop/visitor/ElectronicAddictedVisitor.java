...
public final class ElectronicAddictedVisitor implements ShopVisitor {
  @Override
  public void visitShop(Collection<? extends ShopItem> shop) {
    ...
    for (ShopItem item : shop) {
      if (item instanceof ElectronicItem) {
        ...
        say(
            format(
                "Просто смотрю товар \"%s\" и похоже его мощность %s",
                currentItem.getName(), currentItem.getPower()));

        if (null == electronicWithMaxPower
            || electronicWithMaxPower.getPower() < currentItem.getPower()) {
