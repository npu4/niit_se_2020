...
final public class UI {
    ...
    public boolean isCross(Rectangle a, Rectangle b) {
        ...
        if (
                aX > bX + bW || // если левый край первого лежит правее правого края второго
                bX > aX + aW || // или левый край второго лежит правее правого края первого
                aY > bY + bH || // или верхний край первого лежит ниже нижнего края второго
                bY > aY + aH    // или верхний край второго лежит ниже нижнего края первого
        ) {
            return false;
        }
...
//
//        if (max(aX, aW) < min(bX, bW) || max(aY, aH) < min(bY, bH) || min(aY, aH) > max(bY, bH)) {
//            return false;
//        }
//
